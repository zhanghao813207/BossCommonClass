...
{
    NSString *url = @"https://ccdcapi.alipay.com/validateAndCacheCardInfo.json";
    NSDictionary *para = @{
    ...
    sharedManager.requestSerializer.timeoutInterval = 30.f;
    ...
    [sharedManager GET:url parameters:para progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ([responseObject[@"stat"] isEqualToString:@"ok"]) {
           NSString *bankName = [self bankNameByBankCode:responseObject[@"bank"]];
           successBlock(YES,bankName);
        } else {
            successBlock(YES,@"");
        }
...
+ (NSString *)bankNameByBankCode:(NSString *)bankCode
{
    if (!bankCode) {
        return @"";
    }
