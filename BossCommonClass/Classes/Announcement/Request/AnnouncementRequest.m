...
+ (void)registerSession:(void(^)(void))successBlock fail:(void(^)(void))failBlock {
    NSDictionary *dic = @{@"device_type":@(Device_type_app_ios),@"device_no":[JYCSimpleToolClass getUUID],@"device_model":@(Device_model_other),@"xinge_device_token":kCache.deviceToken};
    NSLog(@"%@",dic);
